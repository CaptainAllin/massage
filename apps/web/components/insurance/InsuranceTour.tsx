...
const TOUR_STEPS: TourStep[] = [
  {
    title: 'Insurance billing workflow',
    description:
      'Create a claim by clicking "New Claim", linking it to a client and their appointment. Claims move through statuses: Draft → Submitted → Pending → Approved → Paid (or Denied / Appealing if rejected). You can generate a CMS-1500 form for each approved claim.',
    tip: 'Track reimbursements separately under each claim — this lets you reconcile the expected vs received amount from the insurer.',
  },
  {
    title: 'Prerequisites: set up providers first',
    description:
      'Before creating claims, add the relevant Insurance Providers in the "Providers" tab. Each provider needs a Payer ID (the insurer\'s EDI identifier), address, and contact details. Once providers are configured, they appear as options when creating new claims.',
    tip: 'Contact the insurer directly or check their EDI enrollment portal to find the correct Payer ID — an incorrect ID will cause claim rejections.',
  },
];

function storageKey(userId: string) {
  return `insurance_tour_dismissed_${userId}`;
}

export function InsuranceTour() {
  const { user } = useAuth();
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#5D4AA8', letterSpacing: '0.8px' }}>
              Insurance guide &middot; Step {step + 1} of {TOUR_STEPS.length}
            </p>
