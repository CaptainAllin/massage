...
export async function POST(
  ...
  try {
    ...
    if (note.status !== 'PENDING_REVIEW') {
      return res.badRequest('Only notes PENDING_REVIEW can be approved');
    }

    // Only the assigned reviewer or business owner can approve
    const isOwner = await prisma.business.findFirst({
      where: { id: businessId, ownerId: user.id },
    });
    if (!isOwner && note.reviewerId !== user.id) {
      return res.forbidden('Only the assigned reviewer can approve this note');
    }
    ...
    await prisma.auditLog.create({
      data: {
        userId: user.id,
        businessId,
        action: 'TREATMENT_NOTE_APPROVED',
        entityType: 'TreatmentNote',
        entityId: id,
        metadata: { approvedById: user.id },
      },
    });
